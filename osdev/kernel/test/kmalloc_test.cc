...
TEST(kmallocTest, SimpleAligned) {
  ...
  const int align_addr = 64;
  for (int i = 0; i < 10; i++) {
    // Alloc i + 20 bytes aligned as 16.
    mem[i] = kaligned_alloc(align_addr, i + 20);
    EXPECT_TRUE((size_t)(mem[i]) % align_addr == 0);
    EXPECT_TRUE(kernel_memory_manager.SanityCheck());
  }
...
TEST(kmallocTest, RandomSmalliAligned) {
  kernel_memory_manager.Reset();
  int mem_size[] = {583, 192, 119, 66, 964, 381, 150, 207, 718, 52};
  int actions[] = {2, 1, 5, 0, 5, 3, 1, 6, 2, 7, 6, 8, 7, 8, 4, 3, 9, 9, 0, 4};
  int aligns[] = {16, 64, 1024, 4, 128, 4, 32, 256, 512, 1024};
  void* mem[10];
  ...
  for (int i = 0; i < 20; i++) {
    int index = actions[i];
    if (!mem[index]) {
      mem[index] = kaligned_alloc(aligns[index], mem_size[index]);
      EXPECT_TRUE((size_t)(mem[index]) % aligns[index] == 0);
    } else {
      kfree(mem[index]);
    }
  }
...
TEST(kmallocTest, RandomMediumAligned) {
  kernel_memory_manager.Reset();
  constexpr int total_allocs = 100;
  int mem_size[total_allocs] = {
      2077, 6958, 8059, 7871, 663,  1689, 6676, 3084, 3320, 330,  6760, 8774,
  ...
  int aligns[total_allocs] = {
      128,   4096,  128,   512,  512,   2048,  32768, 128,   8192,  32768,
      64,    16384, 64,    16,   4,     4096,  4,     4096,  32,    64,
      16384, 2048,  32,    1024, 256,   512,   32768, 32768, 8192,  128,
      8192,  16384, 16384, 1024, 16384, 256,   16384, 1024,  8,     1024,
      32,    32,    8,     4,    2048,  64,    512,   4096,  8192,  1024,
      2048,  8192,  4,     8192, 2,     2048,  256,   16384, 8,     512,
      16384, 64,    4,     1024, 1024,  16384, 8,     1024,  8,     16,
      8192,  256,   4,     2,    32768, 32768, 16,    128,   1024,  128,
      4096,  4,     64,    1024, 64,    16,    2048,  8192,  16384, 128,
      8192,  512,   16,    1024, 1024,  256,   256,   256,   8,     32,
  };
...
TEST(kmallocTest, RandomLargeAligned) {
  kernel_memory_manager.Reset();
  constexpr int total_allocs = 500;
  int mem_size[total_allocs] = {
      8,    9757, 6167, 4753, 6720, 9689, 2824, 5218, 6716, 4019, 45,   7467,
  ...
  int aligns[total_allocs] = {
      16,    512,   1024,  8192,  64,    512,   16,    64,    8192,  8192,
      512,   32,    256,   128,   64,    16384, 1024,  1024,  1024,  512,
      64,    64,    32768, 8192,  32768, 2,     1024,  2048,  8192,  64,
      8192,  4096,  256,   128,   4096,  16,    4,     1024,  4,     32,
      512,   32,    2048,  1024,  32,    16384, 16,    16384, 2048,  64,
      4096,  16384, 512,   8,     16384, 64,    8,     256,   32,    64,
      256,   256,   16,    2,     32,    32768, 16384, 32768, 16,    4096,
      1024,  128,   128,   512,   2,     32768, 2,     8192,  256,   4096,
      8,     256,   32,    128,   16384, 8,     32,    1024,  4096,  256,
