...
void MapSanityCheck(std::HashMap<int, int>& m, int* state, int num) {
  for (int i = 0; i < num; i++) {
    if (state[i] == 0 || state[i] == 3) {
      EXPECT_TRUE(m.find(i) == nullptr);
    } else if (state[i] == 1) {
      EXPECT_TRUE(*m.find(i) == i + 1);
    } else if (state[i] == 2) {
      EXPECT_TRUE(*m.find(i) == 2 * (i + 1));
    }
  }
}

TEST(HashMapTest, RandomMapTestSmall) {
  std::HashMap<int, int> m;
...
TEST(HashMapTest, RandomMapTestLarge) {
  std::HashMap<int, int> m;
